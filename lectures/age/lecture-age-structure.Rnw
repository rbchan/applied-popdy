...
%  \vfill
  \begin{itemize}
    \item Age class survival $s_{i}$, and
%    \item Age class birth rates $b_{i}$
    \item Age class fecundity $f_{i}$
    \item Fecundity is the birth rate ($b_i$) multiplied by survival
    \pause
    \begin{itemize}
      \normalsize
      \item In pre-reproductive surveys, $f_i = b_i \times s_0$
      \item In post-reproductive surveys, $f_i = b_i \times s_i$
    \end{itemize}
