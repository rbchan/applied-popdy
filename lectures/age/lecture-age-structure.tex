...
\begin{frame}[plain]
  \begin{center}
    {\LARGE \bf Age- and stage-structured population models \par}
    \vspace{0.5cm}
    { \Large February 13 \& 18, 2019} \\
    \vfill
%    \fbox{
...
\note{Recall the birth-death model we used earlier. b*N=E(B). How
  stupid is that?}
...
\begin{frame}
  ...
  \Large
  \begin{itemize}[<+->]
    \item Birth and death rates usually depend on age
    \item If so, growth rates will differ between populations with
      different age structures
    \item Some age classes contribute more to population growth than others
    \item This has important management implications
...
%  \large
  ...
  \begin{itemize}
    ...
    \item<3> Fecundity is product of birth rate and offspring survival, $f_i = b_i
      \times s_0$
...
\begin{frame}
  ...
  \large
  \begin{itemize}[<+->]
    \item Age distribution converges to a {\bf stable age
        distribution} when survival and fecundity rates are constant
    \item Stable age distribution is the proportion of individuals in
      each age class when the population converges
    \item Growth rates of each age class differ at first, but converge
      once the stable age distribution is reached
    \item Asymptotic growth rate is $\lambda$ (without subscript)
    \item Growth rate at the stable age distribution is the same for
...
\note{As with single-age exponential populations, an age-structured
  population with stable age distribution exhibits exponential growth
  at a rate that depends on net reproduction and mortality. However,
  the reproduction and mortality parameters of the single-age model are
  replaced here with average reproduction and mortality rates in which
  age-specific values are weighted by cohort proportions in the stable
  age distribution.}
...
\begin{frame}
  ...
  \Large
  \begin{itemize}[<+->]
    \item Matrix models aren't actually ``new'' models
    \item They are the same ``old'' models we have been talking about
    \item But, they make it much easier to compute important
...
\begin{frame}
  \frametitle{Summary}
  \begin{itemize}[<+->]
    \item Vital rates ($s$ and $f$) are usually age-specific
    \item Population growth will depend on age distribution
    \item If vital rates are constant, population will reach stable
      age distribution with constant growth rate $\lambda$
    \item Reproductive value indicates which age class contributes the
      most to population growth
    \item Matrix models are a convenient method used to work with
