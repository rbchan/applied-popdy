...
source("../../rnw2pdf.R")
rnw2pdf("lecture-mark-recap-openpop")
rnw2pdf("lecture-mark-recap-openpop",tangle=TRUE)
@
...
\begin{frame}
  ...
  fecundity, defined as the birth rate multipled by the probability of
  surviving one time interval. \\
  \vfill
  Note that in other applications, recruitment is defined as the
  product of the birth and the probability of surviving to maturity. \\
\end{frame}
