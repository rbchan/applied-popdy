...
%      \end{itemize}
    ...
    \item<5-> Controls (when conducting an experiment)
  \end{enumerate}
\end{frame}
...
%  Since the data are random, the estimates are random
%\end{frame}
...
%   \large
%   \begin{itemize}[<+->]
%     \item We usually don't know the value of the parameter
%     \item So we have to estimate it
%     \item We estimate it using an estimator
%     \item An estimator is a function that returns an estimate
%   \end{itemize}
% \end{frame}

% \begin{frame}
%   \frametitle{Estimators}
%   The best estimate of the population mean is usually the sample mean

%   What is the best estimate of a
% \end{frame}
...
%\begin{frame}[plain]
%  \LARGE \centering But we only have one sample??? \par
%
%\end{frame}
...
%  \bf
  ...
  \vfill
<<N,include=FALSE,echo=FALSE,eval=FALSE>>=
height <- 5.5
heightSD <- 0.5
nSim <- 100
x1 <- x2 <- numeric(nSim)
