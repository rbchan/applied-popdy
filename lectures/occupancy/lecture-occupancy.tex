...
%  \]
  ...
  \begin{itemize}[<+->]
    ...
    \item $p$ -- Probability of detecting at least one individual at a
      site that is occupied during a sampling occasion
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    ...
    \item We assume \alert{population closure}: A site's occupancy
      state does not change during the season
...
\begin{frame}
  \frametitle{How many potholes have mallards?}
  \begin{center}
...
\begin{frame}
  \frametitle{Design}
  \large
  To estimate $\psi$ and $p$, we should follow all the rules we talked
  about in the sampling lecture, plus:
  \begin{itemize}[<+->]
...
\begin{frame}
  ...
  \Large
  {\centering If you sample $K$ times, the overall detection probability ($\bar{p}$) is:}
  \[
    \bar{p} = 1 - (1-p)^K
  \]
  \vfill
...
\begin{frame}
  ...
  \large
  \begin{itemize}[<+->]
    \item Occupancy models let us estimate metapopulation parameters
      when detection is imperfect
    \item Occupancy and detection are site-level parameters, there are
      no individual-level variables
    \item These methods are often easy to implement over large areas
      and so are used in monitoring programs
    \item Definition of site and season are very important considerations
    \item Models can be used in other contexts, such as when a site is
