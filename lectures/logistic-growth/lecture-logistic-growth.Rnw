...
%  \vspace{-0.5cm}
  \begin{itemize}[<+->]
    ...
    \item Strange behavior of the model, e.g. damped oscillations and chaos
...
\begin{frame}
  \frametitle{Density-dependent growth}
  \large
  \textbf{Logistic growth is an example of density-dependent
    growth} \\
  \vspace{0.5cm}
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    ...
    \item Growth ($\Delta_t=N_{t+1}-N_t$) peaks at $K/2$
...
%    \item Soon we will see how the model can be extended to model
%      population cycles, and how it has been used to manage harvest
%      \begin{itemize}
%        \large
%        \item Overcompensation
