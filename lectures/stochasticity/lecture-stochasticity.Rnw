...
sigma.d <- 0.1             ## StdDev of growth rate
for(t in 2:nYears) {
    r[t-1] <- rnorm(n=1, mean=r.bar, sd=sigma.d)
...
\begin{frame}[fragile]
  ...
  \[
     N_{t+1} = N_t + B_t - D_t
  \]
<<pois-bin,fig.width=7,fig.height=5,out.width="0.8\\textwidth",fig.align='center',echo=FALSE>>=
