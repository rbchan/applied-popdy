...
\begin{frame}
  ...
  \large
%  \begin{itemize}[<+->]
%    \item A random variable is a variable whose value can change every time
%      you observe it
%    \item In other words, its value can't be predicted with certainty
%    \item Examples?
%      \begin{itemize}
%        \item Weather
...
%      \end{itemize}
%    \item We use probability distributions to describe random variables.
%    \item Examples include the normal distribution, the binomial distribution,
%      and the Poisson distribution
%    \item In the context of the exponential growth, the growth
%      parameter $r$ might be a normally-distributed random variable
%      with mean $\bar{r}$ and standard deviation (SD) $\sigma$.
%  \end{itemize}
...
%% \begin{frame}
%%   \frametitle{Stochasticity}
%%   \large
%%   {\bf If a future event cannot be predicted with certainty, it is said to
%%   be random. \par}
%%   \pause
%%   \vspace{1cm}
%%   {\bf Do we live in a stochastic world?}
%% \end{frame}
...
legend(-2.1, 3, c("mu=0, sigma^2=0.6",
               "mu=0, sigma^2=0.4",
               "mu=0, sigma^2=0.2"),
...
%  {\bf Sampling error}
%  \begin{itemize}
%    \item Random variation in the data due to FALSE positive or FALSE
%      negative detections
%    \item[]
%  \end{itemize}
\end{frame}
...
%% \begin{frame}[fragile]
%%   \frametitle{Geometic growth}
%% %  \vspace{-0.4cm}
%% <<geom1,fig=TRUE,include=FALSE,echo=FALSE>>=
%% T <- 50
%% N <- integer(T+1)
%% N[1] <- 50
%% for(t in 1:T) {
...
%% @
%% \begin{center}
%%   \includegraphics[width=0.7\textwidth]{stochasticity-geom1}
%% \end{center}
%% \end{frame}
