...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item[]
    \item The maximum yield is $H = r_{max}K/4$
    \item[]
...
\centering
$S = 0.8 - 0.005 \times N$ \\
\includegraphics[width=0.9\textwidth]{figure/phi1-1} \\
\end{frame}
...
plot(function(h) (1-h)*(0.8 - 0.005*(100 - 100*h)), 0, 1,
     ...
     lwd=3, col="blue")
@
<<phibar2,include=FALSE,echo=FALSE,fig.height=6,fig.width=8>>=
plot(function(h) (1-h)*(0.8 - 0.005*(100 - 100*h)), 0, 1,
     ylim=c(0,0.5), xlab="Harvest rate (h)",
     ylab="Overall survival rate",
     main="", ##expression(paste(beta[0], "= 0.8, ", beta[1], "= 0, N=100")),
     lwd=3, col="blue")
...
@
\begin{center}
  \only<1|handout:0>{\includegraphics[width=0.95\textwidth]{figure/phibar-1} \\}
  \only<2>{\includegraphics[width=0.95\textwidth]{figure/phibar2-1} \\}
\end{center}
%\pause
%\vfill
% \small
% {Conclusion}: Because harvest mortality is compensatory, the
