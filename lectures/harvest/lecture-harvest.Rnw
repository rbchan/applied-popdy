...
%  \vspace{2cm}
  \vfill
  \Large
  \centering \bf What value of $H_t$ achieves equilibrium? \par %, i.e. $N_{t+1} = N_t$?
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    ...
    \item If harvest is not compensated for by improved survival, mortality is
      said to be additive
%    \item The probability that an individual survives is often denoted
...
%  $\phi = 0.8333 - 0.005556\times N$
  \vspace{-0.5cm}
<<phi1,echo=FALSE,include=FALSE>>=
plot(function(x) 0.8333 - 0.005556*x, 0, 90, col="blue",
     xlab="Population size (N)",
     ylab="Survival (S)",
     ylim=c(0, 1), lwd=2)
@
\par
  \begin{center}
    \includegraphics[width=0.8\textwidth]{figure/phi1-1}
  \end{center}
\end{frame}
...
%  \large
  ...
  \end{itemize}
  \begin{center}
    \includegraphics[width=0.5\textwidth]{figure/phi1-1}
  \end{center}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Overall survival vs. harvest rate}
  \vspace{-0.5cm}
<<phibar,include=FALSE,echo=FALSE>>=
plot(function(h) (1-h)*(0.8333 - 0.005556*(90 - 90*h)), 0, 1,
     ylim=c(0,1), xlab="Harvest rate (h)",
     ylab="Overall survival rate",
     main=expression(paste(beta[0], "= 0.8333, ", beta[1], "= 0.005556, N=90")),
     lwd=3, col="purple")
@
\begin{center}
