...
copy and paste results from Excel and R to the WORD file. You should
...
computer where {\bf R} will look for files by default. To find out
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{setwd}\hlstd{(}\hlstr{"C:/Users/Richard/Documents/APD/"}\hlstd{)} \hlcom{## You will need to modify the path in quotes}
\end{alltt}
\end{kframe}
\end{knitrout}
...
describing the model that you want to fit. If it is {\tt ~1 ~1}, then
there are no covariates. We will add covariates later. First, let's
...
a dataframe for prediction like this: 
...
Import the file {\tt quali-data-for-R.csv}.

\begin{enumerate}
  \item[(a)] Fit the single-season model using the \texttt{occu}
  ...
  \item[(b)] Now run another model using \texttt{veght} as a
    predictor variable (covariate) for both occupancy and detection
    probability. 
  ...
  \item[(d)] Use the occupancy estimates (on the logit scale) to
    ...
    as described above, and do it in Excel using the provided template.
    Add both graphs to your Word document.
  \item[(e)] Based on your graphs, does occurrence probability
...
\begin{enumerate}
  \item[(a)] Use the \texttt{colext} function to estimate psi ($\psi$), gamma
    ($\gamma$), epsilon ($\epsilon$), 
    and p. Report the 4 estimates and standard errors by creating a
    table in your Word document.  
  \item[(c)] Provide clear interpretations of these estimates. 
