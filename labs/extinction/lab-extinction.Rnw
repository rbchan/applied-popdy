...
Geometric growth with environmental stochasticity. %% The function
%% \inr{rnorm} generates random numbers from the normal distribution.
...
Poisson-Binomial birth-death model. %% The \inr{rpois} function generates
%% random numbers from the Poisson distribution, and \inr{rbinom} is used
