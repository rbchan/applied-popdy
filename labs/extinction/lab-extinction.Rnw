...
\begin{enumerate}
  \item Simulate 10 populations under a logistic growth model with
    $N_0$=20, $K$=40, and a normally-distributed growth rate $rmax_t
...
is a useful model for data that are non-negative integers. It only has
a single parameter (often called ``lambda'' but not to be confused with the
finite rate of increase) and it has no upper bound. In stochastic
...
\[
  B_t \sim \mathrm{Poisson}(b \times N_t)
\]
