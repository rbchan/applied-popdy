...
Undergrads with access to Windows can use program MARK. Everyone else
can use the R package 'mra'. Put your answers in a Word file and
upload it to ELC. Name the file something like ``Chandler-lab10.docx''. 
...
\begin{figure}[h!]
  \centering
  \fbox{\includegraphics[height=0.7\textheight]{figs/stinkpot07-data}}
...
\section*{\large  Part III: Closed-population models in R}
...
Model $M_0$ can be fit like this:
